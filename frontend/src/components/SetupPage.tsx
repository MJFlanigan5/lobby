...
interface ConfigState {
  ...
  HA_LIGHT_ENTITY_ID: string;
  HUE_BRIDGE_IP: string;
  HUE_USERNAME_SET: boolean;
  HUE_LIGHT_ID: string;
  LIFX_TOKEN_SET: boolean;
  LIFX_SELECTOR: string;
  SONOS_IP: string;
  SONOS_DUCK_VOLUME: string;
  LATITUDE: string;
  LONGITUDE: string;
  TEMP_UNIT: string;
  TIMEZONE: string;
    ...
    fetch('/api/config', { headers: { 'X-Lobby-Token': getAuthToken() } })
      ...
      .then((d) => {
        ...
        setForm((f) => ({
          ...
          HA_URL: d.HA_URL || '',
          HA_LIGHT_ENTITY_ID: d.HA_LIGHT_ENTITY_ID || '',
          HUE_BRIDGE_IP: d.HUE_BRIDGE_IP || '',
          HUE_LIGHT_ID: d.HUE_LIGHT_ID || '',
          LIFX_SELECTOR: d.LIFX_SELECTOR || '',
          SONOS_IP: d.SONOS_IP || '',
          SONOS_DUCK_VOLUME: d.SONOS_DUCK_VOLUME || '15',
          LOCATION: d.LOCATION || '',
          LATITUDE: d.LATITUDE || '',
    ...
    <div className="min-h-screen bg-[#0a0a0a] py-12 px-6">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Home Assistant Light Sync (optional)">
            <p className="text-white/30 text-xs leading-relaxed">
              Same ambient color sync as Govee, but through Home Assistant — works with any light HA exposes (Hue, LIFX, Zigbee, groups, etc.), not just Govee. Needs a long-lived access token: HA profile → Security → Long-Lived Access Tokens.
            </p>
            <Field label="Home Assistant URL" id="ha-url" value={form.HA_URL} placeholder="http://homeassistant.local:8123" onChange={set('HA_URL')} />
            <Field
              label={status.HA_TOKEN_SET ? 'Access Token (set)' : 'Access Token'}
              id="ha-token"
              type="password"
              value={form.HA_TOKEN}
              placeholder={status.HA_TOKEN_SET ? 'Leave blank to keep' : 'Long-lived access token'}
              onChange={set('HA_TOKEN')}
              onClear={status.HA_TOKEN_SET ? () => clearSecret('HA_TOKEN') : undefined}
            />
            <Field label="Light Entity ID" id="ha-entity" value={form.HA_LIGHT_ENTITY_ID} placeholder="light.living_room" onChange={set('HA_LIGHT_ENTITY_ID')} />
          </Section>

          <Section title="Philips Hue Sync (optional)">
            <p className="text-white/30 text-xs leading-relaxed">
              Direct to a Hue bridge, no Home Assistant required. The username is an API key generated by pairing with the bridge — press the physical button, then POST to <code className="text-white/40">http://&lt;bridge-ip&gt;/api</code> with <code className="text-white/40">{'{"devicetype":"lobby"}'}</code> within 30 seconds.
            </p>
            <Field label="Bridge IP" id="hue-ip" value={form.HUE_BRIDGE_IP} placeholder="192.168.1.x" onChange={set('HUE_BRIDGE_IP')} />
            <Field
              label={status.HUE_USERNAME_SET ? 'Username / API Key (set)' : 'Username / API Key'}
              id="hue-username"
              type="password"
              value={form.HUE_USERNAME}
              placeholder={status.HUE_USERNAME_SET ? 'Leave blank to keep' : 'From bridge pairing'}
              onChange={set('HUE_USERNAME')}
              onClear={status.HUE_USERNAME_SET ? () => clearSecret('HUE_USERNAME') : undefined}
            />
            <Field label="Light ID" id="hue-light" value={form.HUE_LIGHT_ID} placeholder="1" onChange={set('HUE_LIGHT_ID')} />
          </Section>

          <Section title="LIFX Sync (optional)">
            <p className="text-white/30 text-xs leading-relaxed">
              Uses the LIFX Cloud API. Get a personal access token at{' '}
              <a href="https://cloud.lifx.com/settings" target="_blank" rel="noreferrer" className="text-white/50 underline">cloud.lifx.com/settings</a>.
            </p>
            <Field
              label={status.LIFX_TOKEN_SET ? 'Access Token (set)' : 'Access Token'}
              id="lifx-token"
              type="password"
              value={form.LIFX_TOKEN}
              placeholder={status.LIFX_TOKEN_SET ? 'Leave blank to keep' : 'Personal access token'}
              onChange={set('LIFX_TOKEN')}
              onClear={status.LIFX_TOKEN_SET ? () => clearSecret('LIFX_TOKEN') : undefined}
            />
            <Field label="Selector" id="lifx-selector" value={form.LIFX_SELECTOR} placeholder="all, or label:Living Room" onChange={set('LIFX_SELECTOR')} />
          </Section>

          <Section title="Sonos Volume Ducking (optional)">
            <p className="text-white/30 text-xs leading-relaxed">
              Lowers Sonos volume while Lobby's theme music plays, and restores it after — so the two don't clash. Local control only; find the speaker's IP in the Sonos app or your router.
            </p>
            <Field label="Speaker IP" id="sonos-ip" value={form.SONOS_IP} placeholder="192.168.1.x" onChange={set('SONOS_IP')} />
            <Field label="Ducked Volume (0-100)" id="sonos-duck" value={form.SONOS_DUCK_VOLUME} placeholder="15" onChange={set('SONOS_DUCK_VOLUME')} />
          </Section>
